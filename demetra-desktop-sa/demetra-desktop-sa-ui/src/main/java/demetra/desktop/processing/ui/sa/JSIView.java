...
public final class JSIView extends JComponent implements TimeSeriesComponent, HasTs, HasColorScheme {
    ...
    private void onTsChange() {
        Ts ts = getTs();
        setData(ts != null ? ts.getData() : null);
    }
    ...
    private void displayData(TsData seas, TsData irr, DecompositionMode mode) {
        ...
        for (int p=0; p<freq; ++p) {
            ...
            if (n > 0) {
                ...
                double x = xstart + xstep * (startyear - y0);
                ...
                Graphs g = new Graphs(t2, s2, si2, CalendarUtility.formatPeriod(freq, il));
                graphs_.put(b, g);
    ...
    static void configureAxis(XYPlot plot, int freq) {
        NumberAxis yAxis = new NumberAxis();
        yAxis.setTickLabelPaint(Color.GRAY);
        rescaleAxis(yAxis);
        plot.setRangeAxis(yAxis);
