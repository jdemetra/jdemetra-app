...
/**
 * The class
 * <code>JSurface</code> is responsible for the generation of surface images and
 * user mouse events handling. It relies on a SurfaceModel that handles
 * everything. This class only display data available in the model.
 *
...
public class JSurface extends javax.swing.JComponent {
    ...
    public void setOptimum(SurfaceVertex optimum) {
...
//        surfaceVertex = new SurfaceVertex[];
    ...
    /**
     * Destroys the internal image. It will force
     * <code>SurfaceCanvas</code> to regenerate all images when the
     * <code>paint</code> method is called.
     */
    ...
    /**
     * Sets the data availability flag. If this flag is
     * <code>false</code>,
     ...
     * this flag to
     * <code>false</code> when data is not available.
     *
     ...
     * @param y the y coordinate of cursor
     */
    public void doExportJPG(File file) throws IOException {
        if (file == null) {
            return;
        }
        ...
        export(g2d);
        printAll(g2d);
        javax.imageio.ImageIO.write(bf, "JPG", file);
    }
    ...
    /**
     * Updates image. Just call the
     * <code>paint</code> method to avoid flickers.
     *
    ...
    private synchronized void draw(Graphics graphics) {
        ...
        switch (plot_type) {
            ...
            case WIREFRAME:
                plotWireframe();
                if (optimum != null) {
                    Point tickpos = projector.project(optimum.x, optimum.y, -10f);
                    outPoint(graphics, tickpos.x, tickpos.y);
                }
                break;
            case SURFACE:
                plotSurface();
    ...
    private void outPoint(Graphics g, int x, int y) {
    ...
    private void plotArea(int start_lx, int start_ly, int end_lx, int end_ly, int sx, int sy) {
        ...
        while (ly != end_ly) {
            if (plotfunc1) {
                values1[1] = surfaceVertex[lx + ly];
                values1[2] = surfaceVertex[lx + ly + sy];
            }
            ...
            while (lx != end_lx) {
                ...
                if (plotfunc1) {
                    values1[0] = values1[1];
                    values1[1] = surfaceVertex[lx + sx + ly];
                    values1[3] = values1[2];
                    values1[2] = surfaceVertex[lx + sx + ly + sy];
                }
                if (plotfunc1) {
                    curve = 1;
    ...
    private void plotContour() {
        ...
        if (plotfunc1) {
            ...
            for (int i = 0; i < calc_divisions; i += multiple_factor) {
                ...
                for (int j = 0; j < calc_divisions; j += multiple_factor) {
                    contour_vertex[0] = surfaceVertex[index];
                    contour_vertex[1] = surfaceVertex[index + multiple_factor];
                    contour_vertex[2] = surfaceVertex[index + delta + multiple_factor];
                    contour_vertex[3] = surfaceVertex[index + delta];
                    createContour();
        ...
        if (optimum != null) {
            outPoint(graphics, contour_center_x, contour_center_y);
        }
                    ...
                    while (j <= calc_divisions) {
                        ...
                        if (!invalid) {
                            ...
                            if (z < zmin) {
                                ...
                                projection = projector.project(ratio * (surfaceVertex[k].x - lx) + lx, ratio * (surfaceVertex[k].y - ly) + ly, -10);
                            } else if (z > zmax) {
                                error = true;
                                ...
                                error = false;
                                projection = surfaceVertex[k].projection(projector);
                            }
                            if (lasterror && (!error) && (j != 0)) {
                                if (lastz > zmax) {
                                    float ratio = (zmax - z) / (lastz - z);
                                    lastproj = projector.project(ratio * (lx - surfaceVertex[k].x) + surfaceVertex[k].x, ratio * (ly - surfaceVertex[k].y) + surfaceVertex[k].y, 10);
                                } else if (lastz < zmin) {
                        ...
                        lastproj = projection;
                        lasterror = error;
                        lx = surfaceVertex[k].x;
                        ly = surfaceVertex[k].y;
                        lastz = z;
                        j++;
                        k++;
                    }
