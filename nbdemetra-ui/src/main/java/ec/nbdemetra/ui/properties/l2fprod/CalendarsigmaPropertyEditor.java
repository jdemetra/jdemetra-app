 ...
 * @author s4504ch
 */
public class CalendarsigmaPropertyEditor  extends AbstractPropertyEditor {

    private Sigmavec[] groups_;

    public CalendarsigmaPropertyEditor() {
        editor = new SigmavecEditor();
    }
    
    void fireChanged(Sigmavec[] groups) {
        Sigmavec[] old = groups_;
        groups_ = groups;
    ...
    public void setValue(Object value) {
        if (null != value && value instanceof Sigmavec[]) {
            groups_ = (Sigmavec[]) value;
    ...
    class SigmavecEditor extends JPanel {
                    ...
                    final JTable table = new JTable(
                            new DefaultTableModel() {
                                ...
                                public void setValueAt(Object aValue, int row, int column) {
                                    if (column == 1) {
                                        ngroups_[row] = (Sigmavec) aValue;
                                    }
                                    fireTableCellUpdated(row, column);
                                }
                            });

                    table.setDefaultEditor(Sigmavec.class, new CustomSigmavecEditor());
        ...
        public void setGroups(final Sigmavec[] param) {
...
class CustomSigmavecEditor extends AbstractCellEditor implements TableCellEditor {
    ...
    public CustomSigmavecEditor() {
        DefaultComboBoxModel model = new DefaultComboBoxModel(Sigmavec.values());
