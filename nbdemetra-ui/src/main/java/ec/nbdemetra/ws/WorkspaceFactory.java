...
public class WorkspaceFactory implements LookupListener {
    ...
    private final ArrayList<IWorkspaceRepository> repositories_ = new ArrayList<>();
    private final List<IWorkspaceItemManager> modules_ = new ArrayList<>();
//    private List<WorkspaceItem<?>> tmp_ = new ArrayList<WorkspaceItem<?>>();
    ...
    private WorkspaceFactory() {
        workspaceManagersLookup = Lookup.getDefault().lookupResult(IWorkspaceItemManager.class);
        modules_.addAll(workspaceManagersLookup.allInstances());
        repositoryLookup = Lookup.getDefault().lookupResult(IWorkspaceRepository.class);
        for (IWorkspaceRepository fac : repositoryLookup.allInstances()) {
            fac.initialize();
    ...
    public IWorkspaceItemManager<?> getManager(Id family) {
        for (IWorkspaceItemManager mgr : modules_) {
    ...
    public <T extends IWorkspaceItemManager> T getManager(Class<T> tclass) {
        for (IWorkspaceItemManager mgr : modules_) {
    ...
    public String getActionsPath(Id id) {
        ...
        IWorkspaceItemManager mgr = getManager(id);
        if (mgr != null) {
...
//    }
    public void setActiveWorkspace(Workspace wks, int event) {
        ...
        ws_ = wks;
        ProcessingContext.setActiveContext(ws_.getContext());
        if (event > 0) {
    ...
    public void newWorkspace(IWorkspaceRepository repo) {
    ...
    public boolean openWorkspace(IWorkspaceRepository repo) {
        try {
...
//            g_ws.ResetDirty();
    ...
    public void getManagerIds(List<Id> ids) {
        for (IWorkspaceItemManager mgr : this.modules_) {
            ids.add(mgr.getId());
        }
    }

    public List<IWorkspaceItemManager> getManagers() {
    ...
    public List<IWorkspaceRepository> getRepositories() {
        return Collections.unmodifiableList(repositories_);
    }

    public IWorkspaceRepository getActiveRepository() {
    ...
    public <D extends IWorkspaceRepository> D getRepository(Class<D> dclass) {
        for (IWorkspaceRepository repo : repositories_) {
    ...
    public IWorkspaceRepository getRepository(String name) {
        for (IWorkspaceRepository repo : repositories_) {
