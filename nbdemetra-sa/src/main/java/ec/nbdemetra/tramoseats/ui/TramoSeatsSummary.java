...
public class TramoSeatsSummary extends JComponent implements IDisposable {
    ...
    public void set(TramoSeatsDocument doc) {
        ...
        if (seats == null) {
            document = new HtmlTramoSeatsSummary(doc_.getInput().getName(), doc_.getResults(), null, null, null);
        } else {
            UcarimaModel ucm = seats.getUcarimaModel();
            document = new HtmlTramoSeatsSummary(doc_.getInput().getName(), doc_.getResults(), getComponentsName(ucm), getComponents(ucm), null);
        }
