...
public class PiView extends AChartView {
    ...
    public PiView(ContinuousInformationProvider provider) {
    ...
    private static class CustomCommand extends MatrixChartCommand {
        ...
        protected Matrix toMatrix(ChartPanel chartPanel) {
            ...
            int nr=dataset.getItemCount(0), nc=dataset.getSeriesCount();
            double[] data=new double[nr*(nc+1)];
            for (int k=nr, j=0; j<nc; ++j)
                for (int i=0; i<nr; ++i)
                    data[k++]=dataset.getYValue(j, i);
            return Matrix.of(data, nr, nc+1);
        }
